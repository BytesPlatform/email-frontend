...
export function EmailDraftsList({ 
  ...
  return (
    <div className="space-y-3">
      {drafts.map((draft) => (
        <div key={draft.id} className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md transition-all hover:border-indigo-300">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 truncate">{draft.contactEmail || 'No email'}</p>
                </div>
              </div>
              
              <div className="mt-2">
                <p className="text-sm font-medium text-gray-900 truncate mb-1">{draft.subject}</p>
                {draft.body && (
                  <p className="text-xs text-gray-600 line-clamp-2">{draft.body}</p>
                )}
              </div>
              ...
              <div className="flex items-center gap-3 mt-2 flex-wrap">
                ...
                {draft.status === 'sent' && draft.opens !== undefined && (
                  <span className="text-xs text-gray-500">
                    📧 {draft.opens} {draft.clicks ? `• 📎 ${draft.clicks}` : ''}
                  </span>
                )}
              </div>
            </div>
